...
class UI:
    def __init__(self, window, timers):
        ...
        self.font_timer_name = pygame.font.Font(None, 24)
    ...
    def update_button_positions(self):
        ...
        button_width = (window_width - 5 * BUTTON_SPACING) // 6
        self.start_stop_button_rect = pygame.Rect((window_width - 6 * button_width - 5 * BUTTON_SPACING) // 2, button_y, button_width, BUTTON_HEIGHT)
        ...
        self.prev_timer_button_rect = pygame.Rect(self.add_15_sec_button_rect.right + BUTTON_SPACING, button_y, button_width, BUTTON_HEIGHT)
        self.next_timer_button_rect = pygame.Rect(self.prev_timer_button_rect.right + BUTTON_SPACING, button_y, button_width, BUTTON_HEIGHT)

    def handle_mouse_event(self, pos):
        if self.start_stop_button_rect.collidepoint(pos):
            if self.timers[self.current_timer_index].running:
                self.timers[self.current_timer_index].pause()
            else:
                self.timers[self.current_timer_index].start()
        elif self.reset_button_rect.collidepoint(pos):
            self.timers[self.current_timer_index].reset()
        elif self.add_15_min_button_rect.collidepoint(pos):
            self.timers[self.current_timer_index].add_time(900)
        elif self.add_15_sec_button_rect.collidepoint(pos):
            self.timers[self.current_timer_index].add_time(15)
            ...
            if self.server_running:
                ...
                start_server(self.timers[self.current_timer_index])
    ...
    def draw_buttons(self):
        ...
        pygame.draw.rect(self.window, GREEN2 if self.timers[self.current_timer_index].running else RED, self.start_stop_button_rect)
    ...
    def draw_timer_name(self):
        ...
        window_width, window_height = self.window.get_size()
        timer_name_rect = timer_name_text.get_rect(center=(window_width // 2, window_height // 2 + 50))
